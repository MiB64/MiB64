...
void CompileCop1Test (BLOCK_SECTION * Section) {
	...
	TestVariable(&RecompPos, STATUS_CU1,&STATUS_REGISTER,"STATUS_REGISTER");
...
void Compile_R4300i_LWC1 (BLOCK_SECTION * Section) {
	...
	if (IsConst(Opcode.IMM.base)) { 
		...
		if (Compile_LW(TempReg1, Address)) {
			...
			MoveVariableToX86reg(&RecompPos, &FPRFloatLoadStoreLocation[Opcode.FP.ft], Name, TempReg2);
			MoveX86regToX86Pointer(&RecompPos, TempReg1, TempReg2);
			return;
		}
	...
	if (UseTlb) {
		...
		MoveX86regPointerToX86reg(&RecompPos, TempReg1, TempReg2,TempReg3);
	} else {
		AndConstToX86Reg(&RecompPos, TempReg1,0x1FFFFFFF);
		TempReg3 = Map_TempReg(Section,x86_Any,-1,FALSE);
		MoveN64MemToX86reg(&RecompPos, TempReg3,TempReg1);
	}
	sprintf(Name,"FPRFloatLocation[%d]",Opcode.FP.ft);
	MoveVariableToX86reg(&RecompPos, &FPRFloatLoadStoreLocation[Opcode.FP.ft],Name,TempReg2);
	MoveX86regToX86Pointer(&RecompPos, TempReg3,TempReg2);
}
...
void Compile_R4300i_LDC1 (BLOCK_SECTION * Section) {
	...
	if (IsConst(Opcode.IMM.base)) { 
		...
		if (Compile_LW(TempReg1, Address)) {
			...
			MoveVariableToX86reg(&RecompPos, &FPRDoubleLocation[Opcode.FP.ft], Name, TempReg2);
			AddConstToX86Reg(&RecompPos, TempReg2, 4);
			MoveX86regToX86Pointer(&RecompPos, TempReg1, TempReg2);
	...
	if (UseTlb) {
		...
		TempReg3 = Map_TempReg(Section,x86_Any,-1,FALSE);
		MoveX86regPointerToX86reg(&RecompPos,TempReg1, TempReg2,TempReg3);
		Push(&RecompPos, TempReg2);
		sprintf(Name,"FPRFloatLocation[%d]",Opcode.FP.ft);
		MoveVariableToX86reg(&RecompPos, &FPRDoubleLocation[Opcode.FP.ft],Name,TempReg2);
		AddConstToX86Reg(&RecompPos,TempReg2,4);
		MoveX86regToX86Pointer(&RecompPos,TempReg3,TempReg2);
		Pop(&RecompPos,TempReg2);
		MoveX86regPointerToX86regDisp8(&RecompPos, TempReg1, TempReg2,TempReg3,4);
		sprintf(Name,"FPRFloatLocation[%d]",Opcode.FP.ft);
		MoveVariableToX86reg(&RecompPos,&FPRDoubleLocation[Opcode.FP.ft],Name,TempReg2);
		MoveX86regToX86Pointer(&RecompPos,TempReg3,TempReg2);
	} else {
		...
		MoveN64MemDispToX86reg(&RecompPos,TempReg3,TempReg1,4);
		sprintf(Name,"FPRFloatLocation[%d]",Opcode.FP.ft);
		MoveVariableToX86reg(&RecompPos,&FPRDoubleLocation[Opcode.FP.ft],Name,TempReg2);
		MoveX86regToX86Pointer(&RecompPos,TempReg3,TempReg2);
	}
...
void Compile_R4300i_SWC1 (BLOCK_SECTION * Section){
	...
	if (IsConst(Opcode.IMM.base)) { 
		...
		sprintf(Name,"FPRFloatLocation[%d]",Opcode.FP.ft);
		MoveVariableToX86reg(&RecompPos,&FPRFloatLoadStoreLocation[Opcode.FP.ft],Name,TempReg1);
		MoveX86PointerToX86reg(&RecompPos,TempReg1,TempReg1);
	...
	if (UseTlb) {
		...
		TempReg3 = Map_TempReg(Section,x86_Any,-1,FALSE);
		sprintf(Name,"FPRFloatLocation[%d]",Opcode.FP.ft);
		MoveVariableToX86reg(&RecompPos,&FPRFloatLoadStoreLocation[Opcode.FP.ft],Name,TempReg3);
		MoveX86PointerToX86reg(&RecompPos,TempReg3,TempReg3);
		MoveX86regToX86regPointer(&RecompPos, TempReg3,TempReg1, TempReg2);
	} else {
		TempReg2 = Map_TempReg(Section,x86_Any,-1,FALSE);
		UnMap_FPR(Section,Opcode.FP.ft,TRUE);
		sprintf(Name,"FPRFloatLocation[%d]",Opcode.FP.ft);
		MoveVariableToX86reg(&RecompPos,&FPRFloatLoadStoreLocation[Opcode.FP.ft],Name,TempReg2);
		MoveX86PointerToX86reg(&RecompPos,TempReg2,TempReg2);
		AndConstToX86Reg(&RecompPos,TempReg1,0x1FFFFFFF);
		MoveX86regToN64Mem(&RecompPos, TempReg2, TempReg1);
	}
}
...
void Compile_R4300i_SDC1 (BLOCK_SECTION * Section){
	...
	if (IsConst(Opcode.IMM.base)) { 
		...
		TempReg1 = Map_TempReg(Section,x86_Any,-1,FALSE);
		sprintf(Name,"FPRDoubleLocation[%d]",Opcode.FP.ft);
		MoveVariableToX86reg(&RecompPos,(BYTE *)&FPRDoubleLocation[Opcode.FP.ft],Name,TempReg1);
		AddConstToX86Reg(&RecompPos,TempReg1,4);
		MoveX86PointerToX86reg(&RecompPos, TempReg1,TempReg1);
		Compile_SW_Register(TempReg1, Address);

		sprintf(Name,"FPRDoubleLocation[%d]",Opcode.FP.ft);
		MoveVariableToX86reg(&RecompPos,&FPRDoubleLocation[Opcode.FP.ft],Name,TempReg1);
		MoveX86PointerToX86reg(&RecompPos,TempReg1,TempReg1);
	...
	if (UseTlb) {
		...
		TempReg3 = Map_TempReg(Section,x86_Any,-1,FALSE);
		sprintf(Name,"FPRDoubleLocation[%d]",Opcode.FP.ft);
		MoveVariableToX86reg(&RecompPos,(BYTE *)&FPRDoubleLocation[Opcode.FP.ft],Name,TempReg3);
		AddConstToX86Reg(&RecompPos,TempReg3,4);
		MoveX86PointerToX86reg(&RecompPos, TempReg3,TempReg3);
		MoveX86regToX86regPointer(&RecompPos, TempReg3,TempReg1, TempReg2);
		AddConstToX86Reg(&RecompPos,TempReg1,4);
		...
		AddConstToX86Reg(&RecompPos,TempReg3,4);
		MoveX86PointerToX86reg(&RecompPos,TempReg3,TempReg3);
		MoveX86regToN64Mem(&RecompPos, TempReg3, TempReg1);
		sprintf(Name,"FPRDoubleLocation[%d]",Opcode.FP.ft);
		MoveVariableToX86reg(&RecompPos, (BYTE *)&FPRDoubleLocation[Opcode.FP.ft],Name,TempReg3);
		MoveX86PointerToX86reg(&RecompPos,TempReg3,TempReg3);
		MoveX86regToN64MemDisp(&RecompPos, TempReg3, TempReg1,4);
	}
...
void Compile_R4300i_COP1_MF (BLOCK_SECTION * Section) {
	...
	MoveVariableToX86reg(&RecompPos, (BYTE *)&FPRFloatLoadStoreLocation[Opcode.FP.fs],Name,TempReg);
	MoveX86PointerToX86reg(&RecompPos,MipsRegLo(Opcode.BRANCH.rt),TempReg);
}
...
void Compile_R4300i_COP1_DMF (BLOCK_SECTION * Section) {
	...
	MoveVariableToX86reg(&RecompPos,(BYTE *)&FPRDoubleLocation[Opcode.FP.fs],Name,TempReg);
	AddConstToX86Reg(&RecompPos,TempReg,4);
	MoveX86PointerToX86reg(&RecompPos,MipsRegHi(Opcode.BRANCH.rt),TempReg);
	sprintf(Name,"FPRDoubleLocation[%d]",Opcode.FP.fs);
	MoveVariableToX86reg(&RecompPos,(BYTE *)&FPRDoubleLocation[Opcode.FP.fs],Name,TempReg);
	MoveX86PointerToX86reg(&RecompPos,MipsRegLo(Opcode.BRANCH.rt),TempReg);
}
...
void Compile_R4300i_COP1_CF(BLOCK_SECTION * Section) {
	...
	Map_GPR_32bit(Section,Opcode.BRANCH.rt,TRUE,-1);
	MoveVariableToX86reg(&RecompPos,&FPCR[Opcode.FP.fs],FPR_Ctrl_Name[Opcode.FP.fs],MipsRegLo(Opcode.BRANCH.rt));
}
...
void Compile_R4300i_COP1_MT( BLOCK_SECTION * Section) {	
	...
	if (IsConst(Opcode.BRANCH.rt)) {
		MoveConstToX86Pointer(&RecompPos,MipsRegLo(Opcode.BRANCH.rt),TempReg);
	} else if (IsMapped(Opcode.BRANCH.rt)) {
		MoveX86regToX86Pointer(&RecompPos,MipsRegLo(Opcode.BRANCH.rt),TempReg);
	} else {
		MoveX86regToX86Pointer(&RecompPos, Map_TempReg(Section,x86_Any, Opcode.BRANCH.rt, FALSE),TempReg);
	}
}
...
void Compile_R4300i_COP1_DMT( BLOCK_SECTION * Section) {
	...
	if (IsConst(Opcode.BRANCH.rt)) {
		...
		if Is64Bit(Opcode.BRANCH.rt) {
			MoveConstToX86Pointer(&RecompPos,MipsRegHi(Opcode.BRANCH.rt),TempReg);
		} else {
			MoveConstToX86Pointer(&RecompPos,MipsRegLo_S(Opcode.BRANCH.rt) >> 31,TempReg);
		}
	} else if (IsMapped(Opcode.BRANCH.rt)) {
		MoveX86regToX86Pointer(&RecompPos,MipsRegLo(Opcode.BRANCH.rt),TempReg);
		AddConstToX86Reg(&RecompPos,TempReg,4);
		if Is64Bit(Opcode.BRANCH.rt) {
			MoveX86regToX86Pointer(&RecompPos,MipsRegHi(Opcode.BRANCH.rt),TempReg);
		} else {
			MoveX86regToX86Pointer(&RecompPos, Map_TempReg(Section,x86_Any, Opcode.BRANCH.rt, TRUE),TempReg);
		}
	} else {
		int x86Reg = Map_TempReg(Section,x86_Any, Opcode.BRANCH.rt, FALSE);
		MoveX86regToX86Pointer(&RecompPos,x86Reg,TempReg);
		AddConstToX86Reg(&RecompPos,TempReg,4);
		MoveX86regToX86Pointer(&RecompPos, Map_TempReg(Section,x86Reg, Opcode.BRANCH.rt, TRUE),TempReg);
	}
}
...
void Compile_R4300i_COP1_CT(BLOCK_SECTION * Section) {
	...
	if (IsConst(Opcode.BRANCH.rt)) {
		MoveConstToVariable(&RecompPos,MipsRegLo(Opcode.BRANCH.rt),&FPCR[Opcode.FP.fs],FPR_Ctrl_Name[Opcode.FP.fs]);
	} else if (IsMapped(Opcode.BRANCH.rt)) {
		MoveX86regToVariable(&RecompPos,MipsRegLo(Opcode.BRANCH.rt),&FPCR[Opcode.FP.fs],FPR_Ctrl_Name[Opcode.FP.fs]);
	} else {
		MoveX86regToVariable(&RecompPos,Map_TempReg(Section,x86_Any,Opcode.BRANCH.rt,FALSE),&FPCR[Opcode.FP.fs],FPR_Ctrl_Name[Opcode.FP.fs]);
	}
	Pushad(&RecompPos);
	Call_Direct(&RecompPos, (void*)ChangeDefaultRoundingModel, "ChangeDefaultRoundingModel");
	Popad(&RecompPos);
...
void Compile_R4300i_COP1_S_ADD (BLOCK_SECTION * Section) {
	...
	if (RegInStack(Section,Reg2, FPU_Float)) {
		fpuAddReg(&RecompPos, StackPosition(Section,Reg2));
	} else {
		DWORD TempReg;
		...
		MoveVariableToX86reg(&RecompPos,(BYTE *)&FPRFloatLoadStoreLocation[Reg2],Name,TempReg);
		Load_FPR_ToTop(Section,Opcode.FP.fd,Opcode.FP.fd, FPU_Float);
		fpuAddDwordRegPointer(&RecompPos, TempReg);
	}
...
void Compile_R4300i_COP1_S_SUB (BLOCK_SECTION * Section) {
	...
	if (Opcode.FP.fd == Opcode.FP.ft) {
		...
		TempReg = Map_TempReg(Section,x86_Any,-1,FALSE);
		sprintf(Name,"FPRFloatLocation[%d]",Opcode.FP.ft);
		MoveVariableToX86reg(&RecompPos,(BYTE *)&FPRFloatLoadStoreLocation[Opcode.FP.ft],Name,TempReg);
		fpuSubDwordRegPointer(&RecompPos, TempReg);
	} else {
		Load_FPR_ToTop(Section,Opcode.FP.fd,Reg1, FPU_Float);
		if (RegInStack(Section,Reg2, FPU_Float)) {
			fpuSubReg(&RecompPos, StackPosition(Section,Reg2));
		} else {
			UnMap_FPR(Section,Reg2,TRUE);
			Load_FPR_ToTop(Section,Opcode.FP.fd,Opcode.FP.fd, FPU_Float);
...
void Compile_R4300i_COP1_S_MUL (BLOCK_SECTION * Section) {
	...
	Load_FPR_ToTop(Section,Opcode.FP.fd,Reg1, FPU_Float);
	if (RegInStack(Section,Reg2, FPU_Float)) {
		fpuMulReg(&RecompPos, StackPosition(Section,Reg2));
	} else {
		UnMap_FPR(Section,Reg2,TRUE);
		Load_FPR_ToTop(Section,Opcode.FP.fd,Opcode.FP.fd, FPU_Float);
		...
		sprintf(Name,"FPRFloatLocation[%d]",Reg2);
		MoveVariableToX86reg(&RecompPos,(BYTE *)&FPRFloatLoadStoreLocation[Reg2],Name,TempReg);
		fpuMulDwordRegPointer(&RecompPos, TempReg);
	}
	UnMap_FPR(Section,Opcode.FP.fd,TRUE);
}
...
void Compile_R4300i_COP1_S_DIV (BLOCK_SECTION * Section) {
	...
	if (Opcode.FP.fd == Opcode.FP.ft) {
		...
		sprintf(Name,"FPRFloatLocation[%d]",Opcode.FP.ft);
		MoveVariableToX86reg(&RecompPos,(BYTE *)&FPRFloatLoadStoreLocation[Opcode.FP.ft],Name,TempReg);
		fpuDivDwordRegPointer(&RecompPos, TempReg);
	} else {
		Load_FPR_ToTop(Section,Opcode.FP.fd,Reg1, FPU_Float);
		if (RegInStack(Section,Reg2, FPU_Float)) {
			fpuDivReg(&RecompPos, StackPosition(Section,Reg2));
		} else {
			UnMap_FPR(Section,Reg2,TRUE);
			Load_FPR_ToTop(Section,Opcode.FP.fd,Opcode.FP.fd, FPU_Float);
...
void Compile_R4300i_COP1_S_CMP (BLOCK_SECTION * Section) {
	...
	if (RegInStack(Section,Reg2, FPU_Float)) {
		fpuComReg(&RecompPos, StackPosition(Section,Reg2),FALSE);
	} else {
		DWORD TempReg;
		...
		sprintf(Name,"FPRFloatLocation[%d]",Reg2);
		MoveVariableToX86reg(&RecompPos, (BYTE *)&FPRFloatLoadStoreLocation[Reg2],Name,TempReg);
		fpuComDwordRegPointer(&RecompPos, TempReg,FALSE);
	}
	AndConstToVariable(&RecompPos, (DWORD)~FPCSR_C, &FSTATUS_REGISTER, "FSTATUS_REGISTER");
	fpuStoreStatus(&RecompPos);
	x86reg = Map_TempReg(Section,x86_Any8Bit, 0, FALSE);
	
	if (cmp != 0) {
		TestConstToX86Reg(&RecompPos, cmp,x86_EAX);
		Setnz(&RecompPos, x86reg);
		
		if ((Opcode.REG.funct & 1) != 0) {
			int x86reg2 = Map_TempReg(Section,x86_Any8Bit, 0, FALSE);
			AndConstToX86Reg(&RecompPos, x86_EAX, 0x4300);
			CompConstToX86reg(&RecompPos, x86_EAX, 0x4300);
			Setz(&RecompPos, x86reg2);
	
			OrX86RegToX86Reg(&RecompPos, x86reg, x86reg2);
		}
	} else if ((Opcode.REG.funct & 1) != 0) {
		AndConstToX86Reg(&RecompPos, x86_EAX, 0x4300);
		CompConstToX86reg(&RecompPos, x86_EAX, 0x4300);
		Setz(&RecompPos, x86reg);
	}
	ShiftLeftSignImmed(&RecompPos, x86reg, 23);
	OrX86RegToVariable(&RecompPos, &FPCR[31], "FPCR[31]", x86reg);
}
...
void Compile_R4300i_COP1_D_ADD (BLOCK_SECTION * Section) {
	...
	if (RegInStack(Section,Reg2, FPU_Double)) {
		...
		MoveVariableToX86reg(&RecompPos, (BYTE *)&FPRDoubleLocation[Reg2],Name,TempReg);
		Load_FPR_ToTop(Section,Opcode.FP.fd,Opcode.FP.fd, FPU_Double);
		fpuAddQwordRegPointer(&RecompPos, TempReg);
	}
}
...
void Compile_R4300i_COP1_D_SUB (BLOCK_SECTION * Section) {
	...
	if (Opcode.FP.fd == Opcode.FP.ft) {
		...
		TempReg = Map_TempReg(Section,x86_Any,-1,FALSE);
		sprintf(Name,"FPRDoubleLocation[%d]",Opcode.FP.ft);
		MoveVariableToX86reg(&RecompPos, (BYTE *)&FPRDoubleLocation[Opcode.FP.ft],Name,TempReg);
		Load_FPR_ToTop(Section,Opcode.FP.fd,Opcode.FP.fs,FPU_Double);
		fpuSubQwordRegPointer(&RecompPos, TempReg);
	} else {
		Load_FPR_ToTop(Section,Opcode.FP.fd,Reg1, FPU_Double);
		if (RegInStack(Section,Reg2, FPU_Double)) {
			fpuSubReg(&RecompPos, StackPosition(Section,Reg2));
...
void Compile_R4300i_COP1_D_MUL (BLOCK_SECTION * Section) {
	...
	if (RegInStack(Section,Reg2, FPU_Double)) {
		...
		sprintf(Name,"FPRDoubleLocation[%d]",Reg2);
		MoveVariableToX86reg(&RecompPos, (BYTE *)&FPRDoubleLocation[Reg2],Name,TempReg);
		fpuMulQwordRegPointer(&RecompPos, TempReg);
	}
}
...
void Compile_R4300i_COP1_D_DIV (BLOCK_SECTION * Section) {
	...
	if (Opcode.FP.fd == Opcode.FP.ft) {
		...
		MoveVariableToX86reg(&RecompPos, (BYTE *)&FPRDoubleLocation[Opcode.FP.ft],Name,TempReg);
		Load_FPR_ToTop(Section,Opcode.FP.fd,Opcode.FP.fs,FPU_Double);
		fpuDivQwordRegPointer(&RecompPos, TempReg);
	} else {
		Load_FPR_ToTop(Section,Opcode.FP.fd,Reg1, FPU_Double);
		if (RegInStack(Section,Reg2, FPU_Double)) {
			fpuDivReg(&RecompPos, StackPosition(Section,Reg2));
...
void Compile_R4300i_COP1_D_CMP (BLOCK_SECTION * Section) {
	...
	if (RegInStack(Section,Reg2, FPU_Double)) {
		...
		MoveVariableToX86reg(&RecompPos, (BYTE *)&FPRDoubleLocation[Reg2],Name,TempReg);
		Load_FPR_ToTop(Section,Reg1,Reg1, FPU_Double);
		fpuComQwordRegPointer(&RecompPos, TempReg,FALSE);
	}
	AndConstToVariable(&RecompPos, (DWORD)~FPCSR_C, &FSTATUS_REGISTER, "FSTATUS_REGISTER");
	fpuStoreStatus(&RecompPos);
	x86reg = Map_TempReg(Section,x86_Any8Bit, 0, FALSE);
