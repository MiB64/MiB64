...
static DWORD WriteToAccum2 (int Location, int PC, BOOL RecursiveCall) {
	...
	if (Instruction_State == DELAY_SLOT) { 
		//return TRUE;
		LogMessage("TODO: WriteToAccum2 in delay slot");
	}
	...
	do {
		...
		case DELAY_SLOT:
			if (JumpUncond) {
				...
				LogMessage("TODO: WriteToAccum2 main loop, delay slot, jump uncond");
			} else {
...
BOOL WriteToAccum (int Location, int PC) {
	...
	if (value == HIT_BRANCH) {
		LogMessage("TODO: WriteToAccum hit branch");
		return TRUE; /* ??? */
	} else
		return value;
}
...
static BOOL WriteToVectorDest2 (DWORD DestReg, int PC, BOOL RecursiveCall) {
	...
	if (Instruction_State == DELAY_SLOT) { 
		//return TRUE;
		LogMessage("TODO: WriteToVectorDest2: in delay slot");
	}
...
/*					break;
				...
				LogMessage("TODO: WriteToVectorDest2 main loop, cop2");
			}
			break;
		/*case RSP_LB:
		case RSP_LH:*/
		...
		case DELAY_SLOT: 
			if (JumpUncond) {
				/*PC = JumpTarget - 0x04;
				Instruction_State = NORMAL;*/
				LogMessage("TODO: WriteToVectorDest2 main loop, delay slot, uncond");
			} else {
				Instruction_State = FINISH_BLOCK;
			}
			/*JumpUncond = FALSE;*/
...
BOOL WriteToVectorDest (DWORD DestReg, int PC) {
	...
	if (value == HIT_BRANCH) {
		/*return TRUE;*/ /* ??? */
		LogMessage("TODO: WriteToVectorDest hit branch");
		return FALSE;
	} else
		return value;
}
